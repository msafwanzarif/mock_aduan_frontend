    ...
    <div class="d-flex flex-wrap align-item-center">
      ...
      <div class="card w-25">
        <div class="card-body">
          ...
          <button class="btn btn-outline-danger w-100" @click="$router.push({ name: 'home' })">
            Log Keluar
          </button>
        </div>
      </div>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <input v-if="roleId == 3" v-model="tempIcNo" class="form-control" type="text" />
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" @click="tempNama = nama">
            Batal
          </button>
  ...
  methods: {
    ...
    async saveProfil() {
      try {
        ...
        if (!this.tempNama || !this.tempEmail || (this.roleId == 3 && !this.tempIcNo)) {
          Swal.fire({
            icon: "error",
            title: "Error",
    ...
    getAllMyAduan() {
      axios.request({
        ...
        params: { page: this.currentPage, pageSize: 4, status: this.activeStatus },
        headers: { Authorization: 'Bearer ' + localStorage.getItem('token') }
      }).then((response) => {
        if (response.status == 200) {
